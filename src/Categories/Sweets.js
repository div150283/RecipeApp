...
class Sweets extends Component {
  ...
  async componentDidMount() {
    ...
    let stagedRecipes = 0;
    const sweetsOnlyRecipe = bodyRecipe.map(validRecipe => {
      if(validRecipe.category.id === SWEETS_ENUM) {
        //Only add this recipe if it is from the sweets category.
        stagedRecipes = validRecipe;
      }
      return stagedRecipes;
    });
    this.setState({
      recipes: sweetsOnlyRecipe,
      isLoading: false
    });
  }
  ...
  render() {
    const title = <h3 className="pt-2" style={{display: 'flex', justifyContent: 'center'}}>View Sweets Recipes</h3>
