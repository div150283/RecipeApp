...
class Nonveg extends Component {
  ...
  async componentDidMount() {
    ...
    let stagedRecipes = 0;
    const NonvegOnlyRecipe = bodyRecipe.map(validRecipe => {
      if(validRecipe.category.id === NONVEG_ENUM) {
        //Only add this recipe if it is from the Nonveg category.
        stagedRecipes = validRecipe;
      }
      return stagedRecipes;
    });
    this.setState({
      recipes: NonvegOnlyRecipe,
      isLoading: false
    });
  }

  async remove(id) {
    await fetch('http://localhost:9090/api/recipex/' + id, {
      method: 'DELETE',
      headers: {
  ...
  render() {
    const title = <h3 className="pt-2" style={{display: 'flex', justifyContent: 'center'}}>View Nonveg Recipes</h3>
